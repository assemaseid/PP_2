...
def insert_user_data(username, score, level):
...
def check_wall_collision(snake_pos):
    return snake_pos[0] >= screen_width or snake_pos[0] < 0 or snake_pos[1] >= screen_height or snake_pos[1] < 0
...
while not game_over:
    ...
    if check_wall_collision(snake_pos) or check_self_collision(snake_body):
        ...
        time.sleep(1)
        game_over = True
        insert_user_data(username, score, level)
